...
function make_xml(data_path::String, newick_path::String, xml_path::String,
    ...
    taxa, data = xml.df_to_matrix(df)


    bx = xml.make_MBD_XML(data, taxa, newick, chain_length = 100_000)
    if use_dates
        xml.use_dates!(bx)
    ...
    xml.add_MBD_loggables!(bx)


    xdoc = xml.make_xml(bx)
...
mammals_xml_path = joinpath(xml_dir, "mammals.xml")
...
hiv_xml_path = joinpath(xml_dir, "hiv.xml")
